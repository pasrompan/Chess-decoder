...
if ! command -v bc &> /dev/null; then
    echo "❌ Error: 'bc' command not found. Please install it to run prioritization."
...
calculate_priority() {
    ...
    if [ "$effort" -lt "$MIN_EFFORT" ]; then
        effort=$MIN_EFFORT
    fi
    
    # Calculate: (impact * impact_weight) / (effort * effort_weight) - (dependency_penalty * blocked_count)
    local numerator=$(echo "$impact * $IMPACT_WEIGHT" | bc -l)
    local denominator=$(echo "$effort * $EFFORT_WEIGHT" | bc -l)
...
count_blocked_dependencies() {
    local file="$1"
    local dependencies=$(extract_frontmatter "$file" "dependencies")
    ...
    # Parse dependencies array (format: [ISSUE-001, ISSUE-002] or ["ISSUE-001", "ISSUE-002"])
    local count=0
    # Remove brackets and quotes, split by comma
    local deps=$(echo "$dependencies" | sed 's/^\[//;s/\]$//;s/"//g;s/'\''//g' | tr ',' '\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | grep -v '^$')
    
    while IFS= read -r dep; do
        if [ -n "$dep" ]; then
            # Check if dependency issue exists and is not completed
            local dep_file="$ISSUES_DIR/${dep}.md"
            if [ -f "$dep_file" ]; then
                ...
                # Dependency not found, assume it's blocking
                count=$((count + 1))
            fi
        fi
    done <<< "$deps"
...
for issue_file in "$ISSUES_DIR"/*.md; do
    ...
    # Get numeric values (default to 0 if not set)
    impact=${impact:-0}
    effort=${effort:-1}
    ...
    # Calculate priority
    priority=$(calculate_priority "$impact" "$effort" "$blocked_count")
    
    # Update priority score in file
    update_frontmatter "$issue_file" "priority_score" "$priority"
    
    # Update updated_date
    update_date=$(date +"%Y-%m-%d")
    update_frontmatter "$issue_file" "updated_date" "\"$update_date\""
    
    # Store for sorting
    issue_data+=("$priority|$issue_id|$impact|$effort|$blocked_count")
    
    echo "  ✓ Updated $issue_id: priority=$priority (impact=$impact, effort=$effort, blocked=$blocked_count)"
done
...
for issue_info in "${sorted_issues[@]}"; do
    ...
    if [ -f "$issue_file" ]; then
        ...
        # Format Jira link if available
        if [ -n "$jira_key" ] && [ "$jira_key" != '""' ]; then
            jira_link="[$jira_key]($(extract_frontmatter "$issue_file" "jira_url"))"
        else
            jira_link="-"
        fi
        
        printf "| %.4f | [%s](issues/active/%s.md) %s | %s | %s | %s | %s |\n" \
            "$priority" "$issue_id" "$issue_id" "$jira_link" "$impact" "$effort" "$blocked_count" "$status"
    fi
done
